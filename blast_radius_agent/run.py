...
APP_DIR = os.path.dirname(sys.executable) if IS_FROZEN else SCRIPT_DIR
...
def install_deps():
    ...
    if IS_FROZEN:
        info("Running bundled executable — skipping pip dependency install")
        return
...
def _validate_required_inputs(inputs_dir: str):
    """Validate Jira/GitHub files are present and not placeholder templates."""
    issues = []
    valid_files = []
    ...
    for key, filename in REQUIRED_INPUTS.items():
        ...
        if not os.path.isfile(path):
            issues.append(f"Missing required file: inputs/{filename}")
            continue
        ...
        try:
            ...
            issues.append(f"Cannot read inputs/{filename}: {ex}")
            continue

        if not content:
            issues.append(f"Required file is empty: inputs/{filename}")
            continue

        # Template files usually include this marker from starter content.
        if "[Paste" in content and content.count("\n") < 6:
            issues.append(f"Template not filled yet: inputs/{filename}")
            continue

        valid_files.append(filename)

    return len(issues) == 0, valid_files, issues
...
def main():
    ...
    print(f"  {_c('2', f'{os_emoji} {os_name} • Python {py_ver}')}")
    print()
    ...
    # Step 3: Verify launch mode
    if IS_FROZEN:
        ok("Bundled mode detected")
    else:
    ...
    # If no args, prefer real analysis when required inputs are ready.
    if not args:
        inputs_dir = os.path.join(APP_DIR, "inputs")
        info("No flags specified — checking Jira/GitHub inputs")
        ready, valid_files, issues = _validate_required_inputs(inputs_dir)

        if ready:
            ok(f"Required inputs detected: {', '.join(valid_files)}")
            info("Running real analysis and opening HTML report")
            args = ["--open"]
        else:
            warn("Required inputs are missing or incomplete")
            for issue in issues:
                print(f"    {_c('2', '- ' + issue)}")
            info("Starting demo mode fallback and opening HTML report")
            args = ["--demo", "--open"]
    ...
    if IS_FROZEN:
        # In one-file mode, source files are unpacked to a temp path.
        # Force app I/O paths to the executable folder for demo usability.
        os.environ["BLAST_RADIUS_APP_DIR"] = APP_DIR
        os.chdir(APP_DIR)
